...
#define GPT_SW_PATCH_VERSION_C               4
/** @} */
...
)
{
    ...
    VAR(uint8, AUTOMATIC) u8ErrorId = 0x00U;
#endif
    ...
    {
            u8ErrorId = (uint8)GPT_E_INVALID_CALL;
    }    
    else
    {
        ...
        if (GPT_CHANGE_NEXT_TIMEOUT_VALUE_ID == u8ServiceId) 
        {
            ...
            else if (GPT_CH_MODE_ONESHOT == (*(Gpt_pConfig->Gpt_pChannelConfig))[channel].Gpt_eChannelMode)
            {
                    u8ErrorId = (uint8)GPT_E_PARAM_CHANNEL;
            }         
            else
            {
                /* Do nothing */
...
#if (GPT_SET_CLOCK_MODE == STD_ON) && (GPT_VALIDATE_PARAM == STD_ON)
/**
 * @brief           This function validate the clock mode for a specific channel.
 *
 ...
 * @details
 * @param[in]   u32PredefTimer   PredefTimer
 * @retval      E_OK             The function call is valid
 * @retval      E_NOT_OK         The function call is invalid
 *
 ...
 * @brief       This function validate the initialization pointer.
 *
 * @param[in]   ParamPtr  Pointer to a selected PredefTimer.
 *
 * @return      The validity of the function call
 * @retval      E_OK     The function call is valid
 * @retval      E_NOT_OK The function call is invalid
...
#if (GPT_STM_DEINIT_MODULE == STD_ON)
            Gpt_Ipw_DeInitModule();
#endif
...
*/
FUNC(void, GPT_CODE) Gpt_Channel_EnableChainMode(VAR(Gpt_ChannelType, AUTOMATIC) channel)  
{
#if (GPT_DEV_ERROR_DETECT == STD_ON)
    VAR(Std_ReturnType,AUTOMATIC) returnValue;
#endif
#if (GPT_VALIDATE_CHANNEL_CALL == STD_ON)
    if((Std_ReturnType)E_OK == Gpt_ValidateChannelCall(GPT_ENABLE_CHAIN_MODE_ID,channel))
    {
#endif
          if ((Std_ReturnType)E_OK == Gpt_ValidateChannelStatus(GPT_ENABLE_CHAIN_MODE_ID,channel))
        {
  #if (GPT_DEV_ERROR_DETECT == STD_ON)
            /* Call low level API */
            returnValue = Gpt_Ipw_EnableChainMode(&((*(Gpt_pConfig->Gpt_pChannelConfig))[channel].Gpt_HwChannelConfig));
...
#else   
            /*Call low level API.*/
            (void)Gpt_Ipw_EnableChainMode(&((*(Gpt_pConfig->Gpt_pChannelConfig))[channel].Gpt_HwChannelConfig));
#endif
          }
  #if (GPT_VALIDATE_CHANNEL_CALL == STD_ON)
    }
...
*/
FUNC(void, GPT_CODE) Gpt_Channel_DisableChainMode(VAR(Gpt_ChannelType, AUTOMATIC) channel)  
{
#if (GPT_DEV_ERROR_DETECT == STD_ON)
    VAR(Std_ReturnType,AUTOMATIC) returnValue;
#endif 
#if (GPT_VALIDATE_CHANNEL_CALL == STD_ON)
    if((Std_ReturnType)E_OK == Gpt_ValidateChannelCall(GPT_DISABLE_CHAIN_MODE_ID,channel))
    {
#endif
          if ((Std_ReturnType)E_OK == Gpt_ValidateChannelStatus(GPT_DISABLE_CHAIN_MODE_ID,channel))
        {
  #if (GPT_DEV_ERROR_DETECT == STD_ON)
            /* Call low level API */
            returnValue = Gpt_Ipw_DisableChainMode(&((*(Gpt_pConfig->Gpt_pChannelConfig))[channel].Gpt_HwChannelConfig));
...
#else   
            /*Call low level API.*/
            (void)Gpt_Ipw_DisableChainMode(&((*(Gpt_pConfig->Gpt_pChannelConfig))[channel].Gpt_HwChannelConfig));
#endif 
          }
  #if (GPT_VALIDATE_CHANNEL_CALL == STD_ON)
    }
